...
{   
  ...
  switch (static_cast<action>(p_message.payload[0])) {
    ...
    case action::set_position: {
      auto target_position = -1 * (p_message.payload[1] << 8 | p_message.payload[2]);
    ...
    case action::clamp_power: {
      auto target_speed = p_message.payload[1] << 8 | p_message.payload[2];
      bldc->set_pid_clamped_power(target_speed);
      break;
    }
    ...
    case action::stop:{
      bldc_perseus::PID_settings position_saved = {
        .kp = bldc->get_position_pid_settings().kp,
          //static_cast<float>(p_message.payload[1] << 8 | p_message.payload[2]),
        .ki = bldc->get_position_pid_settings().ki,
          //static_cast<float>(p_message.payload[3] << 8 | p_message.payload[4]),
        .kd = bldc->get_position_pid_settings().kd,
          //static_cast<float>(p_message.payload[5] << 8 | p_message.payload[6])
      };
      bldc_perseus::PID_settings velocity_saved = {
        .kp = bldc->get_velocity_pid_settings().kp,
          //static_cast<float>(p_message.payload[1] << 8 | p_message.payload[2]),
        .ki = bldc->get_velocity_pid_settings().ki,
          //static_cast<float>(p_message.payload[3] << 8 | p_message.payload[4]),
        .kd = bldc->get_velocity_pid_settings().kd,
          //static_cast<float>(p_message.payload[5] << 8 | p_message.payload[6])
      };
      ...
      bldc->update_pid_velocity(hard_stop); 
      // SET FOR ELBOW RIGHT NOW
      // FIX FOR OTHERS
      bldc->update_position(1); 
      bldc->update_pid_position(position_saved); 
      bldc->update_pid_velocity(velocity_saved); 
      break;
    }
