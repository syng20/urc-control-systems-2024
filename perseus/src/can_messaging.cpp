...
{   
  ...
  switch (static_cast<action>(p_message.payload[0])) {
    ...
    case action::stop:{
      bldc_perseus::PID_settings saved = {
        .kp = bldc->get_pid_settings_position().kp,
          //static_cast<float>(p_message.payload[1] << 8 | p_message.payload[2]),
        .ki = bldc->get_pid_settings_position().ki,
          //static_cast<float>(p_message.payload[3] << 8 | p_message.payload[4]),
        .kd = bldc->get_pid_settings_position().kd,
      ...
      // FIX FOR OTHERS
      bldc->update_position(1, 1); 
      bldc->update_pid_position(saved); 
      break;
    }
